...
class AndFilter(FileFilter):
    ...
    def __init__(self, filter1: FileFilter, filter2: FileFilter):
...
class NotFilter(FileFilter):
    ...
    def __init__(self, base_filter: FileFilter):
...
class RegexFilter(FileFilter):
    ...
    def __init__(self, include_patterns: Optional[List[Union[str, Pattern]]] = None,
...
class ContentFilter(FileFilter):
    ...
    def __init__(self, pattern: Union[str, Pattern, Callable[[str], bool]],
                 max_size: Optional[int] = 1024 * 1024,  # Default: 1MB
                 encoding: str = "utf-8"):
        """
        ...
        if isinstance(pattern, str):
            self.matcher = re.compile(pattern).search
