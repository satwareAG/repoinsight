...
async def _run_engine(config: RepoInsightConfig, progress: Progress, task_id: int):
    """
...
def init(
    ...
    try:
        ...
        if config_path.exists():
            confirm = typer.confirm(f"Configuration file already exists at {config_path}. Overwrite?")
            if not confirm:
...
def _load_configuration(config_file: Optional[Path], profile: Optional[str]) -> RepoInsightConfig:
    """
    ...
    # First try to load from specified profile
    if profile:
        try:
    ...
    # Then try to load from specified configuration file
    if config_file:
        try:
    ...
    # Then try to find a configuration file in the current directory
    ...
    # Fallback to default configuration with current directory
