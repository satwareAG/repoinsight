...
class ProcessingEngine:
    ...
    def __init__(self, config: RepoInsightConfig):
        """
        ...
        """
        self.config = config
        self.max_concurrency = min(os.cpu_count() or 4, 
    ...
    def _init_components(self) -> None:
        ...
        if self.config.llm.enabled:
            cache_dir = self.config.get_absolute_cache_path() if self.config.llm.cache_enabled else None
